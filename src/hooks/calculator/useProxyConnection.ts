  ...
  const checkProxyConnection = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `API response status: ${response.status}, ok: ${response.ok}, headers:`,
        ...
        try {
          ...
          throw new Error(
            `Failed to read server response: ${textError.message}`
          );
        }
        ...
        if (isHtmlResponse) {
          ...
          throw new Error(
            "The server is returning HTML instead of JSON. This usually means the Express API routes are not being handled correctly."
          );
        }
      ...
      let errorMessage =
        "Cannot connect to API server. Try refreshing or check your network.";

      if (error.name === "AbortError") {
        errorMessage =
          "Connection timed out. The server might be down or your network might be blocking the connection.";
      } else if (
        error.message &&
  ...
  useEffect(() => {
    ...
    const retryTimer = setInterval(() => {
      if (!proxyConnected && retryCount < MAX_RETRIES) {
        console.log(
          `Auto-retry connection attempt ${retryCount + 1}/${MAX_RETRIES}`
        );
        checkProxyConnection();
      } else {
