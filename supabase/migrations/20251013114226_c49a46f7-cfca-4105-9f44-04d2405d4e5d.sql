-- Fix: Remove 'review' status from needs-enrichment filter
-- This ensures prospects with status='review' ONLY appear in "Needs Review" tab
...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      (p_view = 'needs-enrichment' AND pa.contact_count = 0 AND pa.status = 'enriching') OR
