-- First, add 'interested' to the status check constraint
ALTER TABLE prospect_activities DROP CONSTRAINT IF EXISTS prospect_activities_status_check;

ALTER TABLE prospect_activities ADD CONSTRAINT prospect_activities_status_check 
CHECK (status IN ('new', 'enriching', 'enriched', 'review', 'contacted', 'interested', 'proposal', 'closed_won', 'closed_lost', 'not_viable'));

-- Now migrate all proposal status to interested
UPDATE prospect_activities 
SET status = 'interested', 
    updated_at = NOW()
WHERE status = 'proposal';

-- Update get_crm_prospects_with_stats function to include interested view
